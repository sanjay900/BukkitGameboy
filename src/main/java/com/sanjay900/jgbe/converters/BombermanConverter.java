...
public class BombermanConverter extends Converter{
	...
	public void writeMemory(int address) {
		...
		if (address == 0xc106) {
			...
			int life = cpu.read(address)-1;
			life = life==-1?0:life;
			o.getScoreboard().getTeam("Lives Remaining:").setSuffix(" "+0);
		}
		if (address == 0xc107) {
			o.getScore("Enemies:").setScore(0);
			o.getScoreboard().getTeam("Enemies:").setSuffix(" "+cpu.read(address));
		}
		//TODO: Work out level
		if (address == 0xc058) {
			o.getScore("Level:").setScore(0);
			o.getScoreboard().getTeam("Level:").setSuffix(" "+cpu.read(address));
		}
		//TODO: Work out max bombs
		if (address == 0xc074) {
			o.getScore("Max Bombs:").setScore(0);
			o.getScoreboard().getTeam("Max Bombs:").setSuffix(" "+cpu.read(address));
		}
		if (address == 0xc032||address == 0xc033||address == 0xc034) {
			o.getScore("Time Remaining:").setScore(0);
			o.getScoreboard().getTeam("Time Remaining:").setSuffix(" "+cpu.read(0xc034)+":"+cpu.read(0xc033)+cpu.read(0xc032));
		}
