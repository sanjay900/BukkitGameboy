...
public class MarioLand2Converter extends Converter{
	...
	public void writeMemory(int address) {
		...
		if (address == 0xA262||address == 0xA263) {
			...
			o.getScoreboard().getTeam("Coins:").setSuffix(" "+" "+Integer.toHexString(plugin.cpu.read(0xA263))+Integer.toHexString(plugin.cpu.read(0xA262)));
		}
		if (address == 0xA28D) {
			o.getScore("Enemies Killed:").setScore(0);
			o.getScoreboard().getTeam("Enemies Killed:").setSuffix(" "+plugin.cpu.read(address));
		}
		if (address == 0xA254||address == 0xA255) {
			o.getScore("Time Remaining:").setScore(4);
			o.getScoreboard().getTeam("Time Remaining:").setSuffix(" "+Integer.toHexString(plugin.cpu.read(0xA255))+Integer.toHexString(plugin.cpu.read(0xA254)));
		}
	}
