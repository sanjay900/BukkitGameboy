...
import com.sanjay900.jgbe.emu.tables.*;
...
public final class CPU {
    ...
    private BlockingQueue<Integer> linkCableIn = new LinkedBlockingQueue<>();
    private CPU remoteLink;

    public CPU(GameboyPlayer gp, Player player, ScreenHandler handler) {
        this.gp = gp;
        VC = new VideoController(this, handler);
        AC = new AudioController(this, player);
    }
    ...
    public String first_save_string = "";
    public String last_save_string = "";
    ...
    protected void stateSaveLoad(boolean save, int version, DataOutputStream dostream, DataInputStream distream) throws IOException {
        ...
        if (((10 == -1) || (10 <= version)) && ((-1 == -1) || (version <= -1))) {
            if ((save)) dostream.writeUTF(first_save_string);
            else first_save_string = distream.readUTF();
        }
        if (((10 == -1) || (10 <= version)) && ((-1 == -1) || (version <= -1))) {
            if ((save)) dostream.writeUTF(last_save_string);
            else last_save_string = distream.readUTF();
        }

        if (((1 == -1) || (1 <= version)) && ((-1 == -1) || (version <= -1))) {
            {
                if ((save)) dostream.writeByte((B) & 0xff);
                else B = distream.readUnsignedByte();
            }
            {
                if ((save)) dostream.writeByte((C) & 0xff);
                else C = distream.readUnsignedByte();
            }
            {
                if ((save)) dostream.writeByte((D) & 0xff);
                else D = distream.readUnsignedByte();
            }
            {
                if ((save)) dostream.writeByte((E) & 0xff);
                else E = distream.readUnsignedByte();
            }
        }
        {
            if ((save)) dostream.writeByte((H) & 0xff);
            else H = distream.readUnsignedByte();
        }
        {
            if ((save)) dostream.writeByte((L) & 0xff);
            else L = distream.readUnsignedByte();
        }
        {
            if ((save)) dostream.writeByte((F) & 0xff);
            else F = distream.readUnsignedByte();
        }
        {
            if ((save)) dostream.writeByte((A) & 0xff);
            else A = distream.readUnsignedByte();
        }
        {
            for (int sl_i = 0; sl_i < (0x80); ++sl_i) {
                if ((save)) dostream.writeByte((IOP[sl_i]) & 0xff);
                else IOP[sl_i] = distream.readUnsignedByte();
            }
        }
        {
            for (int sl_i = 0; sl_i < (0x7f); ++sl_i) {
                if ((save)) dostream.writeByte(((HRAM[sl_i]) & 0xff));
                else HRAM[sl_i] = (distream.readUnsignedByte());
            }
        }
        if (((0 == -1) || (0 <= version)) && ((0 == -1) || (version <= 0))) {
            ...
            int skipsize = 0x10000 - 0x1000;
            {
                for (int sl_i = 0; sl_i < (0x1000); ++sl_i) {
                    if ((save)) dostream.writeByte(((WRAM[0][sl_i]) & 0xff));
                    else WRAM[0][sl_i] = (distream.readUnsignedByte());
                }
            }
            if ((!save)) {
                int trysize = 0x8000;
                ...
                if (!doskip)
                    ...
                    int tempskip = 0;
                    for (int sl_i = 0; sl_i < (skipsize - trysize); ++sl_i) {
                        if ((save)) dostream.writeByte(((tempskip) & 0xff));
                        else tempskip = (distream.readUnsignedByte());
                    }
                }
            }
            ...
            for (int i = 1; i < 8; ++i) {
                {
                    for (int sl_i = 0; sl_i < (0x1000); ++sl_i) {
                        if ((save)) dostream.writeByte(((WRAM[i][sl_i]) & 0xff));
                        else WRAM[i][sl_i] = (distream.readUnsignedByte());
                    }
                }
                if (doskip) {
                    int tempskip = 0;
                    for (int sl_i = 0; sl_i < (skipsize); ++sl_i) {
                        if ((save)) dostream.writeByte(((tempskip) & 0xff));
                        else tempskip = (distream.readUnsignedByte());
                    }
                }
        ...
        if (((1 == -1) || (1 <= version)) && ((-1 == -1) || (version <= -1))) {
            for (int sl_i = 0; sl_i < (0x08); ++sl_i)
                for (int sl_j = 0; sl_j < (0x1000); ++sl_j) {
                    if ((save)) dostream.writeByte(((WRAM[sl_i][sl_j]) & 0xff));
                    else WRAM[sl_i][sl_j] = (distream.readUnsignedByte());
                }
        }
        {
            if ((save)) dostream.writeInt(CurrentWRAMBank);
            else CurrentWRAMBank = distream.readInt();
        }
        {
            if ((save)) dostream.writeBoolean(doublespeed);
            else doublespeed = distream.readBoolean();
        }
        {
            if ((save)) dostream.writeBoolean(speedswitch);
            else speedswitch = distream.readBoolean();
        }

        int DIVcntdwnFix = -1;
        if (((-1 == -1) || (-1 <= version)) && ((18 == -1) || (version <= 18))) {
            if ((save)) dostream.writeInt(DIVcntdwnFix);
            else DIVcntdwnFix = distream.readInt();
        }
        if (((19 == -1) || (19 <= version)) && ((-1 == -1) || (version <= -1))) {
            if ((save)) dostream.writeLong(divReset);
            else divReset = distream.readLong();
        }
        int TIMAcntdwnFix = -1;
        if (((-1 == -1) || (-1 <= version)) && ((19 == -1) || (version <= 19))) {
            if ((save)) dostream.writeInt(TIMAcntdwnFix);
            else TIMAcntdwnFix = distream.readInt();
        }
        if (((20 == -1) || (20 <= version)) && ((-1 == -1) || (version <= -1))) {
            if ((save)) dostream.writeLong(TIMAEventCycleCount);
            else TIMAEventCycleCount = distream.readLong();
        }

        if (((-1 == -1) || (-1 <= version)) && ((14 == -1) || (version <= 14))) {
            int tempskip = 0;
            for (int sl_i = 0; sl_i < (1); ++sl_i) {
                if ((save)) dostream.writeInt(tempskip);
                else tempskip = distream.readInt();
            }
        }
        if (((21 == -1) || (21 <= version)) && ((-1 == -1) || (version <= -1))) {
            if ((save)) dostream.writeLong(lastVCRenderCycleCount);
            else lastVCRenderCycleCount = distream.readLong();
        }

        int pc = getPC();
        {
            if ((save)) dostream.writeShort((pc) & 0xffff);
            else pc = distream.readUnsignedShort();
        }
        setPC(pc);

        {
            if ((save)) dostream.writeShort((SP) & 0xffff);
            else SP = distream.readUnsignedShort();
        }
        {
            if ((save)) dostream.writeByte((IE) & 0xff);
            else IE = distream.readUnsignedByte();
        }
        {
            if ((save)) dostream.writeBoolean(IME);
            else IME = distream.readBoolean();
        }
        {
            if ((save)) dostream.writeBoolean(halted);
            else halted = distream.readBoolean();
        }
        if (((16 == -1) || (16 <= version)) && ((-1 == -1) || (version <= -1))) {
            if ((save)) dostream.writeBoolean(delayed_halt);
            else delayed_halt = distream.readBoolean();
        }
        if (((16 == -1) || (16 <= version)) && ((-1 == -1) || (version <= -1))) {
            if ((save)) dostream.writeBoolean(halt_fail_inc_pc);
            else halt_fail_inc_pc = distream.readBoolean();
        }
        ...
        if (((11 == -1) || (11 <= version)) && ((-1 == -1) || (version <= -1))) {
            if ((save)) dostream.writeByte((KeyStatus) & 0xff);
            else KeyStatus = distream.readUnsignedByte();
        }
        if (((12 == -1) || (12 <= version)) && ((-1 == -1) || (version <= -1))) {
            if ((save)) dostream.writeInt(keyBounce);
            else keyBounce = distream.readInt();
        }
        if (((22 == -1) || (22 <= version)) && ((-1 == -1) || (version <= -1))) {
            if ((save)) dostream.writeLong(KeyBounceEventCycleCount);
            else KeyBounceEventCycleCount = distream.readLong();
        }
        int keyBounceWaitNextFix = 0;
        if (((12 == -1) || (12 <= version)) && ((21 == -1) || (version <= 21))) {
            if ((save)) dostream.writeInt(keyBounceWaitNextFix);
            else keyBounceWaitNextFix = distream.readInt();
        }
        if (((-1 == -1) || (-1 <= version)) && ((18 == -1) || (version <= 18))) {
            int tempskip = 0;
            for (int sl_i = 0; sl_i < (1); ++sl_i) {
        ...
        {
            if ((save)) dostream.writeBoolean(BIOS_enabled);
            else BIOS_enabled = distream.readBoolean();
        }

        (cartridge).stateSaveLoad(save, dostream, distream);
        (VC).stateSaveLoad(save, version, dostream, distream);
        if (((2 == -1) || (2 <= version)) && ((-1 == -1) || (version <= -1)))
            (AC).stateSaveLoad(save, version, dostream, distream);
        ...
        if (((3 == -1) || (3 <= version)) && ((8 == -1) || (version <= 8))) {
            if ((save)) dostream.writeInt((int) TotalInstrCount);
            else TotalInstrCount = distream.readInt();
        }
        if (((3 == -1) || (3 <= version)) && ((8 == -1) || (version <= 8))) {
            if ((save)) dostream.writeInt((int) TotalCycleCount);
            else TotalCycleCount = distream.readInt();
        }
        if (((9 == -1) || (9 <= version)) && ((-1 == -1) || (version <= -1))) {
            if ((save)) dostream.writeLong(TotalInstrCount);
            else TotalInstrCount = distream.readLong();
        }
        if (((9 == -1) || (9 <= version)) && ((-1 == -1) || (version <= -1))) {
            if ((save)) dostream.writeLong(TotalCycleCount);
            else TotalCycleCount = distream.readLong();
        }
        ...
        if (((7 == -1) || (7 <= version)) && ((-1 == -1) || (version <= -1))) {
            if ((save)) dostream.writeByte((hblank_dma_state) & 0xff);
            else hblank_dma_state = distream.readUnsignedByte();
        }
    ...
    public void write(int index, int value) {
        ...
        if (mm != null) {
            if (gp != null && gp.conv != null) {
                gp.conv.writtenMemory(index);
            }
        ...
        do {
            ...
            if (LinkCableStatus != 0) {
                ...
                if (LINKcntdwn < 0) {
                    ...
                    remoteLink.linkCableIn.add(lstatus);
                    rstatus = linkCableIn.take();
    ...
    public final void severLink() {
        if (LinkCableStatus != 0) {
            LinkCableStatus = 0;
            this.remoteLink.severLink();
            this.remoteLink = null;
            this.linkCableIn.clear();
        }
    }
    ...
    @SneakyThrows
    public final void clientLink(CPU remoteLink) {
        this.remoteLink = remoteLink;
        remoteLink.remoteLink = this;
        if (LinkCableStatus == 0) {
            LinkCableStatus = 2;
            setDelay(0);
        }
